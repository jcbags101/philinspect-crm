...
export default async function LeadPage({ params }: LeadPageProps) {
  ...
  return (
    <>
      <PageHeader eyebrow="Leads" title={lead.name} description={`${lead.companyName} · ${lead.deletedAt ? "Archived" : label(lead.status)}`} actions={<>{canEdit && <Link className={buttonVariants({ variant: "outline" })} href={`/leads/${lead.id}/edit`}>Edit</Link>}{canArchive && <form action={archiveAction}><Button variant={lead.deletedAt ? "outline" : "destructive"} type="submit">{lead.deletedAt ? "Restore" : "Archive"}</Button></form>}{canEdit && <ConvertLeadButton leadId={lead.id} />}</>} />
